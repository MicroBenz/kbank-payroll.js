...
export default function directCredit(transactions: ITransaction[] = [], config: IConfig) {
  ...
  const header = `HDCT                000000              ${mergedConfig.accountNumber} ${utils.padStart(totalAmount, 15, '0')} ${dayjs(mergedConfig.date).format('YYMMDD')}                         TMT Marketplace Pte. Ltd.                         ${dayjs(mergedConfig.date).format('YYMMDD')}${utils.padStart(transactions.length.toString(), 18, '0')}N     \n`;
